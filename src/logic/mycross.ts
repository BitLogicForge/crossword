...
export class MyCrosswordGenerator {
  ...
  private placeWord(placement: TWordPlacement): void {
    ...
    this.refreshGrid();
    console.log(`Placing word "${placement.word}" at (${placement.x}, ${placement.y}) ${placement.direction})`);
  }
  ...
  public printPossiblePlacements(word: string): void {
    ...
    placements.forEach(placement => {
      console.log(`${placement.word} can be placed at (${placement.x}, ${placement.y}) ${placement.direction}`);
    });
  }
  public printPlacements(): void {
    this.cross.placements.forEach(placement => {
      console.log(`${placement.word} at (${placement.x}, ${placement.y}) ${placement.direction}`);
    });
  }
    ...
    for (let y = 0; y < this.cross.height; y++) {
      for (let x = 0; x < this.cross.width; x++) {
        if (this.cross.gridFilled[y][x] === SYMBOL.EMPTY) {
          this.cross.gridFilled[y][x] = String.fromCharCode(65 + Math.floor(Math.random() * 26));
        }
      }
    }
  }
